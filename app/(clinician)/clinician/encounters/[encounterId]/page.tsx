...
import { CloseEncounterForm } from "@/components/clinician/close-encounter-form";
...
export default async function EncounterReviewPage({ params }: PageProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-6 flex flex-col gap-5">
      ...
      {/* Existing clinician notes if closed */}
      {isClosed && enc.clinicianNotes && (
        <Card>
          <CardContent className="p-5">
      ...
      {/* Close form — only when not yet closed */}
      {!isClosed && <CloseEncounterForm encounterId={encounterId} />}
    </div>
  );
}
